# Actualiza un registro existente a una base de datos MySQL mediante su ID
...
try:
    ...
    '''Paso7: Ejecutar la Sentencia UPDATE'''
    query = "UPDATE personas SET nombre=%s, apellido=%s, edad=%s WHERE id=%s"
    valores = ('Victoria', 'Ramos', 36, 7)

    # Ejecutar la consulta
    cursor.execute(query, valores)
    conexion.commit() # guarda los cambios en la base de datos
    # Obtener todos los resultados
    print(f'Se ha modificado la información del registro en la base de datos por: {valores}')
